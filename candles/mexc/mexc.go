...
// MEXC struct enables requesting candlesticks from MEXC
type MEXC struct {
	apiURL    string
	debug     bool
	lock      sync.Mutex
...
// NewMEXC is the constructor for MEXC
func NewMEXC() *MEXC {
	e := &MEXC{
...
func (e *MEXC) RequestCandlesticks(marketSource common.MarketSource, startTime time.Time, candlestickInterval time.Duration) ([]common.Candlestick, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
