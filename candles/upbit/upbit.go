...
// Upbit struct enables requesting candlesticks from Upbit
type Upbit struct {
	apiURL    string
	debug     bool
	lock      sync.Mutex
...
// NewUpbit is the constructor for Upbit
func NewUpbit() *Upbit {
	e := &Upbit{
...
func (e *Upbit) RequestCandlesticks(marketSource common.MarketSource, startTime time.Time, candlestickInterval time.Duration) ([]common.Candlestick, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
