...
// HTX struct enables requesting candlesticks from HTX (Huobi)
type HTX struct {
	apiURL    string
	debug     bool
	lock      sync.Mutex
...
// NewHTX is the constructor for HTX
func NewHTX() *HTX {
	e := &HTX{
		apiURL: "https://api.htx.com/",
	}
...
func (e *HTX) RequestCandlesticks(marketSource common.MarketSource, startTime time.Time, candlestickInterval time.Duration) ([]common.Candlestick, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
