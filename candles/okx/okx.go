...
// OKX struct enables requesting candlesticks from OKX
type OKX struct {
	apiURL    string
	debug     bool
	lock      sync.Mutex
...
// NewOKX is the constructor for OKX
func NewOKX() *OKX {
	e := &OKX{
...
func (e *OKX) RequestCandlesticks(marketSource common.MarketSource, startTime time.Time, candlestickInterval time.Duration) ([]common.Candlestick, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
