...
// Bitget struct enables requesting candlesticks from Bitget
type Bitget struct {
	apiURL    string
	debug     bool
	lock      sync.Mutex
...
// NewBitget is the constructor for Bitget
...
func (e *Bitget) RequestCandlesticks(marketSource common.MarketSource, startTime time.Time, candlestickInterval time.Duration) ([]common.Candlestick, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
